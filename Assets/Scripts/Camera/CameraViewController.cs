...
namespace GameFrame.Hotfix
{
    public class CameraViewController : MonoBehaviour
    {
        ...
        private Vector3 cameraTempPos = Vector3.zero;

        [Header("Translate")]
        // 鼠标右键 + 上下左右 , 场景漫游
        [SerializeField, Range(1f, 5f)]
        private float translateSensitivity = 1f;                             // 水平灵敏度.
        ...
        {
            cameraTempPos = m_Camera.transform.position + m_Camera.transform.rotation * (Vector3.forward * distance);
        }
        ...
        private void UpdateInput()
        {
            ...
            if (Input.GetKey(KeyCode.Q)) UpInput -= 0.1f;
            if (Input.GetKey(KeyCode.E)) UpInput += 0.1f;
            UpInput = Mathf.Clamp(UpInput, -1, 1);
            if (!Input.GetKey(KeyCode.E) && !Input.GetKey(KeyCode.Q)) UpInput = 0;
        }
        ...
        private void SwitchViewType()
        {
            if (isChangingViewType)
            {
                ...
                {
                    // 切换到正交视图.
                    currentProjMat = UtilsMath.MatrixLerp(currentProjMat, defaultOrthProjMat, viewTypeLerpTime);
                }
                else
                {
                    // 切换到透视视图.
                    currentProjMat = UtilsMath.MatrixLerp(currentProjMat, defaultPersProjMat, viewTypeLerpTime);
                }
        ...
        private void Zoom()
        {
            ...
            if (!UtilsMath.IsEqual(scrollWheelValue, 0f))
            {
                ...
                m_Camera.transform.position = cameraTempPos - m_Camera.transform.rotation * (Vector3.forward * distance);
        ...
        {
            if (rightMouseButton && (VerticalInput != 0 || HorizontalInput != 0|| UpInput!=0))
            {
                cameraTempPos += m_Camera.transform.forward * VerticalInput * translateSensitivity + m_Camera.transform.right * HorizontalInput * translateSensitivity + Vector3.up * UpInput * translateSensitivity;
                m_Camera.transform.position += cameraTempPos;
            }
        ...
        private void LookAround()
        {
                ...
                || (!leftAltKey && !rightAltKey && rightMouseButton))
            {
                ...
                angle_X = UtilsMath.ClampAngle(angle_X, -365, 365);
                angle_Y = UtilsMath.ClampAngle(angle_Y, -365, 365);
        ...
        private void SetCameraPos()
        {
            ...
            m_Camera.transform.position = cameraTempPos + rotation * dir;
        }
        ...
        private void Focus()
        {
            ...
            if (isFocusing)
            {
                ...
                if (focusTime >= 1f)
                {
                    ...
                    // 设置观察中心.
                    cameraTempPos = selectedObj.transform.position;
                }
            }
        }
